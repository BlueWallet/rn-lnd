...
const Native = _reactNative.NativeModules.RnLnd;
...
class RnLndImplementation {
  ...
  initWallet(password = 'gsomgsomgsom', mnemonics) {
    if (this._inited) {
  ...
  async unlockWallet(password = 'gsomgsomgsom') {
    if (this._inited) {
      throw new Error('LND is already inited or unlocked');
    }
  ...
  async startUnlockAndWait(password = 'gsomgsomgsom') {
...
var _default = new RnLndImplementation();
